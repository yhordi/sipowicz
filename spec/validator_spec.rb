describe Validator do
  ...
  describe '#valid_credentials?' do
    ...
    it 'returns true when passed an authentic user' do
      expect(Validator.valid_credentials?(new_user, params[:old_password])).to eq(true)
    end
  end
  describe '#new_passwords_match?' do
    it 'returns true if the user typed their password and confirmation correctly' do
      expect(Validator.new_passwords_match?(params[:user][:password], params[:confirmation])).to eq(true)
    end
