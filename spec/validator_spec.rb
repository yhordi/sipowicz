describe CopDetectiveValidator do
  ...
  describe '#valid_credentials?' do
    ...
    it 'returns true when passed an authentic user' do
      expect(CopDetectiveValidator.valid_credentials?(new_user, params[:old_password])).to eq(true)
    end
  end
  ...
  describe '#validate_new_passwords' do
    describe 'with valid params' do
      it "responds with a success message" do
        CopDetectiveValidator.validate_new_passwords(new_user, params[:user][:password], params[:confirmation])
